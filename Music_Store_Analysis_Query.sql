...
-- C. Return both the city name & sum of all invoice totals.
  ...
  SELECT billing_city, SUM(total) AS Highet_Invoice_Totals
  FROM invoice
  GROUP BY billing_city
  ORDER BY Highet_Invoice_Totals DESC
  LIMIT 1;
...
SELECT customer.customer_id, ANY_VALUE(first_name), ANY_VALUE(last_name), SUM(total) AS total_spending
FROM customer
...
SELECT artist.name AS artist_name, COUNT(invoice_line.quantity) AS purchases
FROM invoice_line 
...
WITH best_selling_artist AS (
	SELECT artist.artist_id AS artist_id, ANY_VALUE(artist.name) AS artist_name, SUM(invoice_line.unit_price*invoice_line.quantity) AS total_sales
	FROM invoice_line
	JOIN track ON track.track_id = invoice_line.track_id
	JOIN album ON album.album_id = track.album_id
	JOIN artist ON artist.artist_id = album.artist_id
